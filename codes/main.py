...
def draw_recs(img, locs, color=(0, 0, 255), lengh=2):
    """
    Draw detected rectanges on input image. Limit the size to reduce false alarm.
    """
...
def check_haar_cascade_success_rate():
    ...
    print('The success rate is: %s%%' % (success/total * 100) )
...
def do_classification():
    ...
    '''
    print('Performing classifiction...')
    ...
    time2 = time.time()
    print('Time Collapse: %s sec' % round(time2 - time1, 2))
    print('Faces saved to: ' + save_path) 


def do_contouring():
    '''
    Do contouring to my dataset and save the result images.
    ...
    time2 = time.time()
    print('Time Collapse: %s sec' % round(time2 - time1, 2))
    print('Contours saved to: ' + save_path)        
